...
<div class="max-w-7xl mx-auto">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">データ編集</h2>
    
    <div class="bg-white rounded-lg shadow p-6">
        <form id="editForm" action="{{ route('data.update', $data) }}" method="POST">
            @csrf
            @method('PUT')
            
            <div class="mb-4">
                <label for="title" class="block text-sm font-medium text-gray-700 mb-2">タイトル</label>
                <input type="text" name="title" id="title" value="{{ old('title', $data->title) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('title')
            ...
            <div class="mb-6">
                ...
                <textarea name="content" id="content" rows="5" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('content', $data->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
...
<script>
setupFormConfirm('editForm', 'データを更新します。よろしいですか？');
</script>
@endsection
